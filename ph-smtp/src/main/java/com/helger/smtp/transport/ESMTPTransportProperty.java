 ...
 * <a href= "https://javamail.java.net/nonav/docs/api/com/sun/mail/smtp/package-summary.html"
 * >Source</a>
 *
...
public enum ESMTPTransportProperty
{
   ...
   * msg.getFrom() or InternetAddress.getLocalAddress(). NOTE: mail.smtp.user was previously used
   * for this.
   */
  ...
  /**
   * Local address (host name) to bind to when creating the SMTP socket. Defaults to the address
   * picked by the Socket class. Should not normally need to be set, but useful with multi-homed
   * hosts where it's important to pick a particular local address to bind to.
   */
  LOCALADDRESS ("localaddress", String.class),

  /**
   * Local port number to bind to when creating the SMTP socket. Defaults to the port number picked
   * by the Socket class.
   */
  LOCALPORT ("localport", int.class),

  /**
   * If false, do not attempt to sign on with the EHLO command. Defaults to true. Normally failure
   * of the EHLO command will fallback to the HELO command; this property exists only for servers
   * that don't fail EHLO properly or don't implement EHLO properly.
   */
  ...
  /**
   * If set, lists the authentication mechanisms to consider, and the order in which to consider
   * them. Only mechanisms supported by the server and supported by the current implementation will
   * be used. The default is "LOGIN PLAIN DIGEST-MD5 NTLM", which includes all the authentication
  ...
  /**
   * The submitter to use in the AUTH tag in the MAIL FROM command. Typically used by a mail relay
   * to pass along information about the original submitter of the message. See also the
   * setSubmitter method of SMTPMessage. Mail clients typically do not use this.
   */
  SUBMITTER ("submitter", String.class),

  /**
   * The NOTIFY option to the RCPT command. Either NEVER, or some combination of SUCCESS, FAILURE,
  ...
  /**
   * If set to true, and the server supports the 8BITMIME extension, text parts of messages that use
   * the"quoted-printable" or"base64" encodings are converted to use"8bit" encoding if they follow
  ...
  /**
   * If set to true, attempt to use the javax.security.sasl package to choose an authentication
   * mechanism for login. Defaults to false.
   */
  ...
  /**
   * The authorization ID to use in the SASL authentication. If not set, the authentication ID (user
   * name) is used.
   */
   ...
   * true (the default), causes the transport to wait for the response to the QUIT command.
   */
  QUITWAIT ("quitwait", boolean.class),

  /**
   * If set to true, causes the transport to include an SMTPAddressSucceededException for each
   * address that is successful. Note also that this will cause a SendFailedException to be thrown
   * from the sendMessage method of SMTPTransport even if all addresses were correct and the message
   ...
   * used to create SMTP sockets. Note that this is an instance of a class, not a name, and must be
   * set using the put method, not the setProperty method.
   */
  ...
  /**
   * If set to true, failure to create a socket using the specified socket factory class will cause
   * the socket to be created using the java.net.Socket class. Defaults to true.
   */
  SOCKETFACTORY_FALLBACK ("socketFactory.fallback", boolean.class),

  /**
   * Specifies the port to connect to when using the specified socket factory. If not set, the
   * default port will be used.
   */
  SOCKETFACTORY_PORT ("socketFactory.port", int.class),

  /**
   * If set to true, use SSL to connect and use the SSL port by default. Defaults to false for
   * the"smtp" protocol and true for the"smtps" protocol.
   */
  SSL_ENABLE ("ssl.enable", boolean.class),

  /**
   * If set to true, check the server identity as specified by RFC 2595. These additional checks
   * based on the content of the server's certificate are intended to prevent man-in-the-middle
   * attacks. Defaults to false.
   */
  SSL_CHECKSERVERIDENTITY ("ssl.checkserveridentity", boolean.class),

  /**
   * If set, and a socket factory hasn't been specified, enables use of a MailSSLSocketFactory. If
   * set to"*", all hosts are trusted. If set to a whitespace separated list of hosts, those hosts
   * are trusted. Otherwise, trust depends on the certificate the server presents.
   */
  SSL_TRUST ("ssl.trust", String.class),

  /**
   * If set to a class that extends the javax.net.ssl.SSLSocketFactory class, this class will be
   * used to create SMTP SSL sockets. Note that this is an instance of a class, not a name, and must
  ...
  /**
   * Specifies the SSL cipher suites that will be enabled for SSL connections. The property value is
   * a whitespace separated list of tokens acceptable to the
   * javax.net.ssl.SSLSocket.setEnabledCipherSuites method.
   */
  SSL_CIPHERSUITES ("ssl.ciphersuites", String.class),

  /**
   * If true, enables the use of the STARTTLS command (if supported by the server) to switch the
   * connection to a TLS-protected connection before issuing any login commands. Note that an
  ...
  /**
   * If true, requires the use of the STARTTLS command. If the server doesn't support the STARTTLS
   * command, or the command fails, the connect method will fail. Defaults to false.
   */
  STARTTLS_REQUIRED ("starttls.required", boolean.class),

  /**
   * Specifies the host name of a SOCKS5 proxy server that will be used for connections to the mail
   * server. (Note that this only works on JDK 1.5 or newer.)
   */
  ...
  /**
   * Extension string to append to the MAIL command. The extension string can be used to specify
   * standard SMTP service extensions as well as vendor-specific extensions. Typically the
   * application should use the SMTPTransport method supportsExtension to verify that the server
   * supports the desired service extension. See RFC 1869 and other RFCs that define specific
   * extensions.
   */
  MAILEXTENSION ("mailextension", String.class),

  /**
   * If set to true, use the RSET command instead of the NOOP command in the isConnected method. In
   * some cases sendmail will respond slowly after many NOOP commands; use of RSET avoids this
   * sendmail issue. Defaults to false.
   */
  USERSET ("userset", boolean.class),

  /**
   * If set to true (the default), insist on a 250 response code from the NOOP command to indicate
   * success. The NOOP command is used by the isConnected method to determine if the connection is
   * still alive. Some older servers return the wrong response code on success, some servers don't
   * implement the NOOP command at all and so always return a failure code. Set this property to
   * false to handle servers that are broken in this way. Normally, when a server times out a
   * connection, it will send a 421 response code, which the client will see as the response to the
   * next command it issues. Some servers send the wrong failure response code when timing out a
   * connection. Do not set this property to false when dealing with servers that are broken in this
   * way.
   */
  ...
  /**
   * @return The property name for regular SMTP transfer. Neither <code>null</code> nor empty.
   */
  @NonNull
  @Nonempty
  ...
  /**
   * @return The property name for secure SMTP transfer. Neither <code>null</code> nor empty.
   */
  @NonNull
  @Nonempty
