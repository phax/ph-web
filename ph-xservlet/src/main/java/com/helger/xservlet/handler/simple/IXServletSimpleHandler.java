...
public interface IXServletSimpleHandler extends IXServletBasicHandler
{
  ...
  /**
   * This callback method is unconditionally called before the last-modification checks are
   * performed. So this method can be used to determine the requested object from the request. This
   * method is not called if HTTP version or HTTP method are not supported.
   *
   * @param aRequestScope
   *        The request scope that will be used for processing the request. Never <code>null</code>.
   * @param aUnifiedResponse
   *        The response object to be filled. Never <code>null</code>.
   * @return {@link EContinue#BREAK} to stop processing (e.g. because a resource does not exist),
   *         {@link EContinue#CONTINUE} to continue processing as usual.
   */
  @OverrideOnDemand
  ...
  /**
   * Get the last modification date time for the current request. If it was not modified since the
   * last request time, a 304 (not modified) response code is returned. This method is always called
   * for GET and HEAD requests. This method is called after
   * {@link #initRequestState(IRequestWebScopeWithoutResponse, UnifiedResponse)} .
   *
   * @param aRequestScope
   *        The request scope that will be used for processing the request. Never <code>null</code>.
   * @return <code>null</code> if no last modification date time can be determined
   */
  @Nullable
  ...
  /**
   * Get the ETag supported for this request. If an ETag matches, a 304 (not modified) response code
   * is returned. This method is always called for GET and HEAD requests.
   *
   * @param aRequestScope
   *        The request scope that will be used for processing the request. Never <code>null</code>.
  ...
  /**
   * Called before a valid request is handled. This method is only called if HTTP version matches,
   * HTTP method is supported and sending a cached HTTP response is not an option.
   *
   * @param aRequestScope
   *        The request scope that will be used for processing the request. Never <code>null</code>.
   */
   ...
   * {@link #handleRequest(IRequestWebScopeWithoutResponse, UnifiedResponse)}. This method is only
   * called for non-request-cancel operations.
   *
   * @param aRequestScope
   ...
   * @return {@link EContinue#CONTINUE} to propagate the Exception, {@link EContinue#BREAK} to
   *         swallow it. May not be <code>null</code>.
   */
  @OverrideOnDemand
  @NonNull
  ...
  /**
   * Called after a valid request was processed. This method is only called if the handleRequest
   * method was invoked. If an exception occurred this method is called after
   * {@link #onException(IRequestWebScopeWithoutResponse, UnifiedResponse, Throwable)}
   *
   * @param aCaughtException
   *        <code>null</code> if no exception occurred, non-<code>null</code> in case of exception.
   */
  @OverrideOnDemand
